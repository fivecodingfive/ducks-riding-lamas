...
class A2CAgent:
    ...
    def train_iterate(self, states, actions, rewards, next_states, dones):
        ...
        dones:        Tensor [T]
        """
        states      = tf.convert_to_tensor(states,      dtype=tf.float32)  # shape [T, D]
        next_states = tf.convert_to_tensor(next_states, dtype=tf.float32)  # shape [T, D]
        rewards     = tf.convert_to_tensor(rewards,     dtype=tf.float32)
        dones       = tf.convert_to_tensor(dones,       dtype=tf.float32)
        actions     = tf.convert_to_tensor(actions,     dtype=tf.int32)
        ...
        adv = returns - values
        with tf.GradientTape() as tape_actor:
            probs = self.actor(states)  # shape [T, action_dim]
            clipped_probs = tf.clip_by_value(probs, 1e-8, 1.0)

            # Gather the log-prob of the selected actions
            idx = tf.stack([tf.range(T), actions], axis=1)
            pi_a = tf.gather_nd(clipped_probs, idx)
            logp = tf.math.log(pi_a)

            # Entropy term
            entropy = -tf.reduce_sum(clipped_probs * tf.math.log(clipped_probs), axis=1)

            # Total loss
            actor_loss = -tf.reduce_mean(logp * tf.stop_gradient(adv) + self.alpha * entropy)
    ...
    def save_model(self, variant):
        ...
        index = len(existing)
        file_name = f"A2Cmodel_{index}_v{variant}.keras"
